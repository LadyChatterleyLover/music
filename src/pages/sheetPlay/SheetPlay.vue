...
<script>
  export default {
    ...
    mounted() {
      this.$com.req(`api/playlist/detail?id=${this.sheetId}`).then(res => {
        console.log(res)
        this.picUrl = res.playlist.coverImgUrl
        this.userName = res.playlist.creator.nickname
        this.publishTime = this.$moment(res.playlist.trackUpdateTime).format('YYYY-MM-DD')
