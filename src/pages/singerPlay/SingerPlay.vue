    ...
    <div class="wrapper" ref="wrapper">
      <div class="w-content">
        ...
        <div class="t-name">
          ...
          <div class="alias" v-for="(item1, index1) in singer.alias" :key="index1">
            ({{item1}})
          </div>
        </div>
        <div class="content">
          ...
          <div class="item" v-for="(item, index) in playList" :key="index" @click="goToPlay(item, index)">
            ...
            <div class="i-desc">
              ...
              <div class="i-singer">
                <div class="singer" v-for="(item2, index2) in item.artists" :key="index2">
                  {{item2.name}} <span v-if="index2 !== item.artists.length - 1">&nbsp;/&nbsp;</span>
...
<script>
  export default {
    ...
    mounted() {
      this.$com.req(`api/artist/desc?id=${this.singerId}`).then(res => {
        this.introduction = res.introduction
      })
      this.$com.req(`api/search?keywords=${this.singName}`).then(res => {
    ...
    computed: {
      ...
      singName () {
        return this.$store.state.singName
      },
      singer () {
